...
const STATUS_STYLE = { "Lançada": "bg-slate-100 text-slate-600", "Faturada": "bg-blue-100 text-blue-700", "Recebida": "bg-green-100 text-green-700", "Em atraso": "bg-red-100 text-red-700" };
...
export default function ProjetosParcelas({ data, onRefresh }) {
  ...
  const [form, setForm] = useState({ os_id: "", valor: "", data_vencimento: "", status: "Lançada" });
  ...
  const emAtraso = filtradas.filter(p => p.status !== "Recebida" && p.data_vencimento && new Date(p.data_vencimento) < new Date()).reduce((s, p) => s + (p.valor || 0), 0);
  ...
  const handleSave = async () => {
    ...
    await base44.entities.Parcela.create({ ...form, valor: parseFloat(form.valor), cliente_nome: proj?.cliente_nome || "", proposta_id: proj?.proposta_id || "" });
    setForm({ os_id: "", valor: "", data_vencimento: "", status: "Lançada" });
    setShowForm(false);
    onRefresh();
    setSaving(false);
  };

  const handleStatus = async (parcela, novoStatus) => { await base44.entities.Parcela.update(parcela.id, { status: novoStatus }); onRefresh(); };
  const handleDelete = async (id) => { await base44.entities.Parcela.delete(id); onRefresh(); };

  return (
    <div className="p-6 space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: DollarSign, color: "border-l-blue-400 text-blue-400", label: "Total portfólio", value: fmt(total) },
          { icon: TrendingUp, color: "border-l-green-500 text-green-500", label: "Faturado/Recebido", value: fmt(faturado) },
          { icon: CreditCard, color: "border-l-amber-400 text-amber-400", label: "A faturar", value: fmt(pendente) },
          { icon: AlertTriangle, color: "border-l-red-400 text-red-400", label: "Em atraso", value: fmt(emAtraso) },
        ].map(({ icon: Icon, color, label, value }) => (
          <Card key={label} className={`border-l-4 ${color.split(" ")[0]}`}>
            <CardContent className="p-4 flex items-center gap-2">
              <Icon size={18} className={color.split(" ")[1]} />
              <div><p className="text-base font-bold text-slate-800">{value}</p><p className="text-xs text-slate-400">{label}</p></div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Button size="sm" onClick={() => setShowForm(!showForm)} className="gap-1.5 bg-[#1A4731] hover:bg-[#245E40]"><Plus size={13} /> Nova Parcela</Button>
      </div>
                  ...
                  return (
                    <tr key={p.id} className={`border-b border-slate-50 hover:bg-slate-50 ${vencida ? "bg-red-50/30" : ""}`}>
                      ...
                      <td className={`px-4 py-2.5 ${vencida ? "text-red-500 font-medium" : "text-slate-500"}`}>
                        {p.data_vencimento ? new Date(p.data_vencimento + "T00:00:00").toLocaleDateString("pt-BR") : "—"}{vencida && " ⚠"}
                      </td>
                      <td className="px-4 py-2.5">
                        <Select value={p.status} onValueChange={v => handleStatus(p, v)}>
                          <SelectTrigger className="h-6 w-28 text-xs border-0 p-0">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLE[p.status] || ""}`}>{p.status}</span>
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="Lançada">Lançada</SelectItem>
                            <SelectItem value="Faturada">Faturada</SelectItem>
                      ...
                      <td className="px-4 py-2.5"><button onClick={() => handleDelete(p.id)} className="text-slate-300 hover:text-red-400"><Trash2 size={12} /></button></td>
                    </tr>
                  );
                })}
                {filtradas.length === 0 && <tr><td colSpan={5} className="text-center py-10 text-slate-300">Nenhuma parcela encontrada</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
