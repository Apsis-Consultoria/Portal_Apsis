...
const TEMP_ICON = { "Quente": "🔴", "Morna": "🟡", "Fria": "🔵" };

export default function ProjetosPipeline({ data }) {
  ...
  const filtradas = propostas.filter(p => {
    ...
    const matchBusca = !busca || [p.cliente_nome, p.numero_ap, p.responsavel, p.natureza].some(v => v?.toLowerCase().includes(text));
    const matchStatus = filtroStatus === "todos" || p.status === filtroStatus;
    const matchTemp = filtroTemp === "todos" || p.temperatura === filtroTemp;
    return matchBusca && matchStatus && matchTemp;
  });

  const totalPipeline = propostas.filter(p => ["Enviada","Em elaboração"].includes(p.status)).reduce((s, p) => s + (p.valor_total || 0), 0);
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-l-4 border-l-blue-400"><CardContent className="p-4"><p className="text-lg font-bold text-slate-800">{fmt(totalPipeline)}</p><p className="text-xs text-slate-400">Pipeline ativo</p></CardContent></Card>
        <Card className="border-l-4 border-l-green-500"><CardContent className="p-4"><p className="text-lg font-bold text-slate-800">{fmt(totalGanho)}</p><p className="text-xs text-slate-400">Total ganho</p></CardContent></Card>
        <Card className="border-l-4 border-l-orange-400">
          <CardContent className="p-4 flex items-center gap-2">
            <AlertTriangle size={18} className="text-[#F47920]" />
            <div><p className="text-lg font-bold text-slate-800">{semFollowup.length}</p><p className="text-xs text-slate-400">Aguardando follow-up</p></div>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-purple-400"><CardContent className="p-4"><p className="text-lg font-bold text-slate-800">{propostas.length}</p><p className="text-xs text-slate-400">Total propostas</p></CardContent></Card>
      </div>
        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              <AlertTriangle size={16} className="text-[#F47920] mt-0.5 flex-shrink-0" />
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {semFollowup.length > 5 && <span className="text-xs text-slate-500">+{semFollowup.length - 5} mais</span>}
                </div>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="text-left px-4 py-3 text-slate-500 font-medium">Temp.</th>
                  <th className="text-left px-4 py-3 text-slate-500 font-medium">Follow-up</th>
              ...
              <tbody>
                {filtradas.map(p => {
                  ...
                  const diasFollowup = p.ultimo_followup ? Math.ceil((new Date() - new Date(p.ultimo_followup)) / (1000 * 60 * 60 * 24)) : null;
                  ...
                  return (
                    <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">{TEMP_ICON[p.temperatura] || "—"} {p.temperatura || "—"}</td>
                      <td className={`px-4 py-3 ${followupAtrasado ? "text-red-500 font-medium" : "text-slate-500"}`}>
                        {p.ultimo_followup ? `${diasFollowup}d atrás` : "Nunca"}{followupAtrasado && " ⚠️"}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLE[p.status] || "bg-slate-100 text-slate-600"}`}>{p.status}</span>
                      </td>
                      <td className="px-4 py-3">
                        {osVinculada ? (
                          <Link to={`/ProjetoDetalhe?id=${osVinculada.id}`} className="flex items-center gap-1 text-[#1A4731] hover:underline text-xs">
            ...
            {filtradas.length === 0 && <div className="text-center py-10 text-slate-300 text-sm">Nenhuma proposta encontrada</div>}
          </div>
        </CardContent>
      </Card>
    </div>
